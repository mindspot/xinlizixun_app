...
public class CounselServiceAdapter extends EasyAdapter<WorkInfoBean> {
    ...
    private boolean isTaoUse = false;
    ...
    class ItemHolder extends EasySimpleViewHolder {
        ...
        public void setData(WorkInfoBean data) {
            super.setData(data);
            lineview.setVisibility(position != 0 && datas.get(position - 1).getGoodsClass() != datas.get(position).getGoodsClass()
                    ? View.VISIBLE : View.GONE);
            moneyText.setText(data.getGoodsName());
            timeText.setText(data.getShareText());
            wayText.setText(data.getMode());
            if (data.getGoodsClass() == 6) {
                itemBtn.setText(isTaoUse ? "套餐卡支付" : "购买套餐卡");
            } else {
                itemBtn.setText(isTaoUse ? "套餐卡支付" : "点击预约");
            }
            itemBtn.setBackgroundResource(data.getGoodsClass() == 6 ?
                    (isTaoUse ? R.drawable.common_gary_background_share : R.drawable.common_theme_background_share)
