...
public class CounselFragment extends BaseFragment {
    ...
    protected void initListener() {
        ...
        refreshLayout.setOnRefreshLoadMoreListener(new OnRefreshLoadMoreListener() {
            ...
            public void onRefresh(RefreshLayout refreshLayout) {
                ...
                cityPopWindow.reset(false);
                timePopWindow.reset(false);
        ...
        cityPopWindow.setOnSelectCityListener(new SelectCityPopWindow.OnSelectCityListener() {
            @Override
            public void onSelect(CityItemBean province, CityItemBean city, boolean isLoad) {
                if (province != null) {
    ...
    public void getHeaderBean() {
        HttpApi.app().getCounselHeaderInfo(this, new BaseRequestBean(), new HttpCallback<CounselHeaderResponseBean>() {
            ...
            public void onSuccess(int code, String message, CounselHeaderResponseBean data) {
                ...
                if (!isHeaderLoadSuccess && mSortIds != null && !mSortIds.isEmpty()) {
                    sortPopWindow.setId(mSortIds.get(0));
                }
    ...
    public void onEventMainThread(TabEvent event) {
        if (event.getType() == TabEvent.TYPE_CHANGE_TAB && event.getTabId() == MainActivity.TAB_INDEX_ZIXUN) {
            if (mSortIds == null) {
                mSortIds = new ArrayList<>();
            }
            mSortIds.clear();
            mSortIds.add(event.getTagId());
            if (isHeaderLoadSuccess) {
                sortPopWindow.setId(event.getTagId());
                getData();
            }
        }
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser && !isHeaderLoadSuccess) {
            getHeaderBean();
        }
    }
}
