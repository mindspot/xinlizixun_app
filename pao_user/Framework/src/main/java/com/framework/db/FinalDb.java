...
import com.framework.db.sqlite.CursorUtils;
import com.framework.db.sqlite.DbModel;
import com.framework.db.sqlite.ManyToOneLazyLoader;
import com.framework.db.sqlite.OneToManyLazyLoader;
import com.framework.db.sqlite.SqlBuilder;
import com.framework.db.sqlite.SqlInfo;
import com.framework.db.table.KeyValue;
import com.framework.db.table.ManyToOne;
import com.framework.db.table.OneToMany;
...
public class FinalDb {
	...
	private FinalDb(DaoConfig config) throws Exception {
		if (config == null)
	...
	public boolean saveBindId(Object entity) {
		...
		if (entityKvList != null && entityKvList.size() > 0) {
			TableInfo tf = null;
			try {
				tf = TableInfo.get(entity.getClass());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private SQLiteDatabase createDbFileOnSDCard(String sdcardPath,
			String dbfilename) throws Exception {
		...
		if (!dbf.exists()) {
			try {
				...
				throw new Exception("数据库文件创建失败", ioex);
			}
		} else {
			return SQLiteDatabase.openOrCreateDatabase(dbf, null);
		}
