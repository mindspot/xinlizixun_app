<template>
  <div class="app-container">
    <el-row type="flex" justify="right" style="padding-bottom:5px ">
      <el-col :span="8">
        <el-button-group>
          <el-button v-show="false" size="mini" type="primary" icon="el-icon-plus" round @click="handleAddClick">新增</el-button>
    ...
    <el-table ref="multipleTable" :data="objectList" tooltip-effect="dark" style="width: 100%" border fit highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="val" align="center" label="内容" width="576" />
      <el-table-column prop="type" align="center" label="类型" width="114">
        <template slot-scope="scope">
          <el-button-group>
            <span v-if="scope.row.type === 300">提现须知</span>
            <span v-if="scope.row.type === 1001">入驻须知</span>
            <span v-if="scope.row.type === 2001">咨询服务须知</span>
            <span v-if="scope.row.type === 2002">支付详细说明</span>   
            <span v-if="scope.row.type === 3001">公司介绍</span>
            <span v-if="scope.row.type === 6001">督导服务须知</span>
          </el-button-group>
        </template>
      </el-table-column>
      <el-table-column align="center" label="操作" width="148">
        <template slot-scope="scope">
          <el-button-group>
            <el-button size="mini" plain @click="handleEditClick(scope.row)">确认</el-button>
    ...
    <el-dialog title="新增信息" :visible.sync="dialogFormVisible" :close-on-press-escape="false" :close-on-click-modal="false" top="15vh" width="48%">
...
<script>
import { commonContentList , addCommon ,deleteCommon, updateCommon } from '@/api/common'
...
export default {
  ...
  methods: {
    getData() {
      ...
      commonContentList(this.param).then(r => {
    ...
    handleAddClick() {
      ...
      this.object = { sort: 10 , type: this.param.type}
      this.ifView = false
