          ...
          <el-col :span="10">
            <el-form ref="form" :model="setMelOrder" label-width="100px" style="width:100%">
              <el-form-item label="咨询类型" required>
                <span v-for="item in commons" :key="'commons'+item.id">{{ item.val }},</span>
              </el-form-item>
            </el-form>
          </el-col>
        </el-row>
        <el-row v-for="item in diagnosisVOs" :key="'diagnosisVOs'+item.id" :gutter="20">
          <el-col :span="10">
              ...
              <el-form-item label="图像" required>
                <el-row type="flex">
                  <el-image
                    ...
                    :preview-src-list="[baseUrl+item.content]"
                  />
                </el-row>
              </el-form-item>
            </el-form>
         ...
         <el-row :gutter="20">
          <el-col :span="10">
            <el-form ref="form" :model="diagnosisVO" label-width="100px" style="width:100%">
              <el-form-item label="描述" required>
                <div class="value">{{ diagnosisVO.content}}</div>
              </el-form-item>
            </el-form>
          </el-col>
        </el-row>
...
<script>
import { setMelaOrderByOrderNo, setMelaOrderByOrderNoDetails, memberCaseByOrderNo, orderCaseClass, diagnosis, diagnos} from '@/api/order'
...
export default {
  ...
  mounted() {
    ...
    this.companyDiagnosis(orderNo)
    this.companyDiagnos(orderNo)
  },
  methods: {
    ...
    companyDiagnosis(orderNo) {
      diagnosis(orderNo).then(r => {
        this.diagnosisVOs = r.result
      })
    },
    companyDiagnos(orderNo) {
      diagnos(orderNo).then(r => {
        if (r.result != null) {
          this.diagnosisVO = r.result
        }
      })
    },
