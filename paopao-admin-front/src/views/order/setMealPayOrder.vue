  ...
  <div class="app-container">
    ...
    <el-table ref="multipleTable" :data="objectList" tooltip-effect="dark" style="width: 100%" border fit highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="168">
        <template slot-scope="scope">
          <el-button v-show="scope.row.orderStatus === 10" plain size="small" @click="changeStatus(scope.row)">退卡次</el-button>
...
<script>
import { orderList, updateCardNumber } from '@/api/order'
  ...
  methods: {
    ...
    changeStatus(item) {
      this.$confirm('该订单是否需要要退款?', '提示', {
        ...
        updateCardNumber(item.orderNo).then(r => {
          if (r.success) {
            this.$message({
              message: '退卡次成功',
              type: 'success'
