<template>
  <div id="service">
    ...
    <div class="footer-container">
      <van-button :loading="isLoading" class="footer" loading-type="spinner" @click="next">保存</van-button>
    </div>
  </div>
</template>>
<script>
import { listTargetPeople, listExpertiseArea } from '@/api/option'
import { settledInUpdateWord, listWorkingTime } from '@/api/consultant'
...
export default {
  ...
  methods: {
    next() {
      ...
      param.removeWorks = this.removeConsultPriceList
      this.isLoading = true
      settledInUpdateWord(param).then(r => {
        this.isLoading = false
        if (!r.success) {
          this.$notify(r.message)
        } else {
          this.$toast.success('提交审核')
    ...
    minusRow(index) {
      if (this.consultPriceList[index].goodsId > 0) {
        this.removeConsultPriceList.push(this.consultPriceList[index])
      }
      this.consultPriceList.splice(index, 1)
    },
    plusRow() {
      if (this.consultPriceList.length < 8) {
        const consultPrice = { goodsId: 0 }
        this.consultPriceList.push(consultPrice)
      } else {
