...
public class ServicePhoneActivity extends BaseActivity {
    ...
    public void initView(Bundle savedInstanceState) {
        try {
            List<ServicePhoneResponseBean.DataBean> phone = MyApplication.app.getServicePhoneResponseBean().getConsultantCustomers();
            List<ServicePhoneResponseBean.DataBean> service = MyApplication.app.getServicePhoneResponseBean().getConsultantCustomersNow();
            if (phone.size() > 0) {
    ...
    public void gotoChat(String userid, int index) {
        IMHolder.getInstance().gotoChat(context(), userid, "在线客服" + index, "", false);
    }
}