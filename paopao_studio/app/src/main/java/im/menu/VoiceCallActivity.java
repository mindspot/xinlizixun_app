...
public class VoiceCallActivity extends CallActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        IMHolder.getInstance().isVoiceCalling = true;
        callType = 0;
        ...
        headerImage = (ImageView) findViewById(R.id.swing_card);
        ...
        nickTextView.setText(IMHolder.getInstance().getUserNickname(username));
        if (StringUtil.isBlank(IMHolder.getInstance().getUserPhoto(username)))
            GlideImageLoader.loadGlideImageCustomCorner(this, IMHolder.getInstance().getUserPhoto(username), headerImage, 30);
    ...
    protected void onDestroy() {
        IMHolder.getInstance().isVoiceCalling = false;
        stopMonitor();
        if (EventBus.getDefault().isRegistered(this)) {
...
//                                        callStateTextView.setText(st7);
    ...
    public void onEventMainThread(ServiceStopEvent event) {
        ToastUtil.show(getApplicationContext(), event.getMsg());
        isRefused = true;
        refuseBtn.setEnabled(false);
        handler.sendEmptyMessage(MSG_CALL_REJECT);
    }
    ...
    public void onEventMainThread(ServiceWillStopEvent event) {
        if (tipDialog == null) {
            tipDialog = new TipDialog(VoiceCallActivity.this);
        }
        tipDialog.setTitle("泡泡心理");
        tipDialog.setContent(event.getMsg());
